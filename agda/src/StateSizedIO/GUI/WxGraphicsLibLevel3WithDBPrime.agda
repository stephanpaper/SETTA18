...
module StateSizedIO.GUI.WxGraphicsLibLevel3WithDBPrime where
...
data DBCommand  : Set where
  connect : String → DBCommand
  prepare : Connection → String → DBCommand
  execute : Statement → List SqlVal → DBCommand
  fetchAllRowsAL' : Statement → DBCommand
  run : Connection → String → List SqlVal → DBCommand
  printList : List (List (String × SqlVal)) → DBCommand



DBCommandResponse : DBCommand → Set
DBCommandResponse (connect str) = Connection
DBCommandResponse (prepare conn str) = Statement
DBCommandResponse (execute statement list) = ℤ
DBCommandResponse (fetchAllRowsAL' statement) = List (List (String × SqlVal))
DBCommandResponse (run conn str sqlVals) = ℤ
DBCommandResponse (printList list) = Unit



--DBCommandResponse _ = Unit


translateDBLocal :  (c : DBCommand)
                      → NativeIO (DBCommandResponse c)
translateDBLocal (connect str) = nativeConnectDB str
translateDBLocal (prepare conn str) = nativePrepare conn str
translateDBLocal (execute statement list) = nativeExecute statement list
translateDBLocal (fetchAllRowsAL' statement) = nativeFetchAllRowsAL' statement
translateDBLocal (run conn str sqlVals) = nativeRun conn str sqlVals
translateDBLocal (printList list) = nativePutStrLn (nativePrintList list)



data GuiLev1Command  : Set where
  dbCmd        : DBCommand → GuiLev1Command
  putStrLn     : String  →  GuiLev1Command
  getLine      : GuiLev1Command
  ...
  getTextFromTxtBox  : TextCtrl → GuiLev1Command
...
GuiLev1Response  (dbCmd c)         = DBCommandResponse c
...
GuiLev1Response  (getTextFromTxtBox _)   = String
GuiLev1Response  getLine                 = String
...
translateLev1Local (dbCmd c)              = translateDBLocal c
...
-- Example
--
example : IO GuiLev1Interface ∞ Unit
example =
  exec (dbCmd (connect "test1.db"))                      λ conn      →
--  exec (dbCmd (execute  conn))                           λ _      →
