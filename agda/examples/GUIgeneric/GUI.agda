...
log : {A : Set} → String → (f : Unit → IO GuiInterface ∞ A) →
         IO GuiInterface ∞ A
log s f = if logOn then (exec (putStrLn s) λ x → f x) else f unit
...
HandlerObject i g = IOObjectˢ  GuiInterface  handlerInterface i g

-- type of the methods of HandlerObject i fr
HandlerIOType : (i : Size)(fr : Frame) → Set
HandlerIOType i fr = IO GuiInterface ∞
                        (Σ[ r ∈ returnType fr ]
...
initHandlerMVar : (l : VarList) (g : Frame)
              ...
              → IOˢ GuiLev2Interface ∞
                    ...
                    s ≡ addVar mvar l  ) l
initHandlerMVar l g a f = execˢ (createVar {_}{StateAndGuiObj} (g , a , f)) λ mvar →
                         returnˢ (mvar , refl)
...
initFFIMVar : (g : Frame)
              ...
              → IOˢ GuiLev2Interface ∞
                    ...
                    s ≡ SingleMVar mvar  ) []
initFFIMVar g comp = execˢ (createVar {_}{StateAndFFIComp} (g , comp)) λ mvar →
                        returnˢ (mvar , refl)
...
module _ where
  ...
  createComp {frame} (createC fr) enc nothing =
     execˢ (level1C createWxFrame) λ frFFI → returnˢ (refl , frFFI)
  ...
  createComp {atomicComp} (createC (button x)) enc maybeFr =
    execˢ (level1C (createButton (enc tt) x)) λ bt →
    returnˢ (refl , bt)

  createComp {atomicComp} (createC (txtbox x)) enc maybeFr =
    execˢ (level1C (createTextCtrl (enc tt) x)) λ txt →
    returnˢ (refl , txt)
...
module _ where
  ...
  buttonHandleraux2 : ∀{i} → (bt  : ButtonComp)
                         ...
                         (obj : HandlerObject ∞ g)
                         (r    : returnType g)
                         (obj' : IOObjectˢ GuiInterface handlerInterface ∞
                                (nextStateFrame g r))
                         → IO GuiInterface i StateAndGuiObj
  ...
  buttonHandleraux2 (createC x) g ffi rel m a obj (changedGUI gNew a') obj' =
                    ...
                    exec (fireCustomEvent (button2Frame (createC x) g ffi rel)) λ _ →
                    return (gNew , a' , obj')
  ...
  setHandler atomicComp (createC (button x)) g ffi (in2= .(createC (button x)) e' .g x₁ rel)  mvar restList =
     execˢ (setButtonHandler (FFIcomponentsReduce (createC (button x)) e' x₁
  ...
  deleteComp {atomicComp} (createC (button x)) ffiButton =
    execˢ (level1C (deleteButton ffiButton)) λ _ →
    returnˢ (refl , _)

  deleteComp {atomicComp} (createC (txtbox y)) ffiTextCtrl =
    execˢ (level1C (deleteTextCtrl ffiTextCtrl)) λ _ →
    returnˢ (refl , _)
  ...
  setAttributes {frame} i (createC fr) (a , b , c , d) x =
    ...
    exec (setChildredLayout x a b c d) λ _ →
    return tt
  ...
  setAttributes {atomicComp} i (createC (button x)) propColor ffiButton =
    exec (setAttribButton ffiButton propColor) λ _ →
    return tt

  setAttributes {atomicComp} i (createC (txtbox y)) propColor ffiTextCtrl =
    exec (setAttribTextCtrl ffiTextCtrl propColor) λ _ →
    return tt
