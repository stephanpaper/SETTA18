...
open import GUIgeneric.PreludeGUI -- renaming (addButton to addButton')
...
--open import StateSizedIO.GUI.WxGraphicsLibLevel3 hiding (addButton) --WithDB hiding (addButton;main)
...
--open import Data.Unit hiding (_≟_; decSetoid; setoid)
...
handlerGUIObjectₛ i S s2GUI s = IOObjectˢ- GuiInterface (handlerGUIInterfₛ S s2GUI) i s
...
mutual
  ...
  hGuiObjₛ2ordaux' : (j : Size)(j' : Size)(S : Set)
                  ...
                  (m   : methodsG (s2GUI s))
                  (prog : IO' GuiInterface j
                            (ReturnTypeₛ S))
                  → IO' GuiInterface j
                         ( Σ[ r ∈ returnType (s2GUI s) ]
                            (IOObjectˢ GuiInterface handlerInterface j'
                             (nextˢ handlerInterface (s2GUI s) m r)))
  hGuiObjₛ2ordaux' j j' S s2GUI s s2prop obj m (exec' c f) = exec' c (λ r → hGuiObjₛ2ordaux j j' S s2GUI s s2prop obj m  (f r) )
